...
const flaps = "<:flaps:417475705912426496>";
...
function geist(text){
	...
	let fflaps = "<:fflaps:615450761899409418>";
    ...
    .replace(new RegExp(flaps, "g"), fflaps)
    ...
    .replace("`|         |`", "`|    flap    |`")
    .replace("\n<", "\n      <");
  let line1 = text.match(/[^>]+bet <a:flappy:849005099761139732> [0-9,]+/gi);
	if(line1){
		text = text.replace(line1[0],"");
		text += "\n"+line1[0].trim();
	}
	let line2 = text.match(/  and won <a:flappy:849005099761139732> [0-9,]+/gi);
	if(line2){
