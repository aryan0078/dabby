...
class SetReferid extends Command {
  ...
  async run(msg, [user]) {
    ...
    if (invited.referid) {
      return msg.send(
        `**${msg.author.username} |** you are invited by <@${invited.referid}> already and he got the rewards already`
      );
    } else {
      if (inviter.invites) {
    ...
    return msg.send(
      `**${msg.author.username} |** you got **1000** <:dabs:851218687255773194> for setting up referid and <@${member.id}> will get **2500** <:dabs:851218687255773194> for inviting you`
    );
  }
}
