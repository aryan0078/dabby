...
const {getCurrency, withdrawBalance} =require("../../structures/database.js");
class Store extends Command {
  ...
  async sell(msg, args) {
    ...
    return msg.send(
      `Successfully sold the role **${
        role.name
      }** for **${refund.toLocaleString()}** dabs ${dabs.currencyEmoji} refund.`
    );
  }
  ...
  async buy(msg, args) {
    ...
    return msg.send(
      `Successfully bought the role **${
        role.name
      }** for **${price.toLocaleString()}** dabs ${dabs.currencyEmoji}`
    );
  }
  ...
  async add(msg, [price, ...args]) {
    ...
    if (price >= Number.MAX_SAFE_INTEGER)
      return msg.send(" That price is too high.");
