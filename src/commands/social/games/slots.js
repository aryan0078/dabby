...
class Slots extends Command {
  ...
  try{  const slots = [
    ...
    if (args.length == 0)
      amount = 1;
    else if (parseInt(args[0]) && args.length == 1)
    ...
    if (maxBet && amount > maxBet)
      amount = maxBet;
    if (result.amount == undefined || result.amount < amount || result.amount <= 0) {
      replyError(msg,"**🚫 | " + msg.author.username + `**, You don't have enough ${dabs.currencyEmoji} ${dabs.currencyName}!`, 3000);
    } else {
      //Decide results
