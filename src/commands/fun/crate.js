  ...
  async run(msg, args) {
    ...
    if (args == "open") {
      if (!msg.member.settings.gender) {
        return replyError(
          msg,
          "**Please set your gender before opening crates**\nuse `dab sg`",
          8000
        );
      }

      let tid = await openCrate(msg, db);
      let ticket = await evalTicket(tid, db);
      ...
      if (!tid) {
        ...
        return replyError(
          msg,
          `You recived **${toFancyNum(500000)}** dabs`,
          8000
        );
      }
      let embed = new MessageEmbed().setImage(
        "https://media.giphy.com/media/5YrT02HhIpbiqFbF4j/giphy.gif"
      );
      if (!ticket) {
        return msg
          .send(
