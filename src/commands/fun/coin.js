...
class Coin extends Command {
  ...
  async run(msg, args) {
  try { //let con=p.con
    var flaps = await getCurrency(msg.guild.id)
...
const tails = "<:coin:849002406124716113>";
    ...
    if (result == undefined || result.amount == 0 || (bet != "all" && result.amount < bet)) {
     replyError(
       msg,
       "**🚫 | " + msg.author.username + `**, You don't have enough ${result.currencyEmoji} !`,3000
     );
      return;
    } else {
      ...
      if (maxBet && bet > maxBet) {
        ...
        replyError(msg, `, you don't have any ${result.currencyEmoji}  silly!`, 3000);
      ...
      text += "\nThe coin spins... " + spin;
      win
        ? await withdrawBalance(msg.author.id, msg.guild.id, bet * 2, false)
        :await withdrawBalance(msg.author.id, msg.guild.id,bet, true);
      ...
      msg.send(
        `I guess you dont have enough ${flaps} use dab convert <amount> to convert your dabs`
      );
    }
  }
}
