...
class Starboard extends Command {
  ...
  async run(msg, [action, amount]) {
    if (!action)
      return msg.send(
        " Specify one of `enable #channel`, `disable` or `limit <amount>`"
      );
    ...
    if(action === "enable") {
      if (!msg.mentions.channels.size)
        return msg.send(" Specify the channel you want to enable it on.");
    ...
    if(action === "limit") {
      ...
      if (amount < 1) return msg.send(" Limit cannot be less than 1");
      if (amount > msg.guild.memberCount)
        return msg.send(
          " Limit cannot be more than the amount of members in the server."
        );
    ...
    return msg.send(
      " Invalid action. Specify one of `enable #channel`, `disable` or `limit <amount>`"
    );
  }
}
