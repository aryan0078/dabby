...
class UserPrefix extends Command {
  ...
  async add(msg, args) {
    if(msg.author.settings.prefix && msg.author.settings.prefix.length >= 10)
      return msg.send(
        " You can't have more than 10 prefixes. Remove some before trying again."
      );
    
    const prefixInput = args.join(" ").toLowerCase();
    if (!prefixInput) return msg.send(" You must provide a prefix.");

    // User prefixes get an extra 5 chars compared to guild prefixes.
    if (prefixInput.length > 15)
      return msg.send(" Prefix cannot be longer than 15 characters!");

    // Get existing prefixes to append to.
    const prefix = msg.author.settings.prefix || [];

    // Avoid duplicates.
    if (prefix.includes(prefixInput))
      return msg.send(" That prefix is already on the list.");
  ...
  async remove(msg, args) {
    ...
    const prefixInput = args.join(" ").toLowerCase();
    if (!prefixInput) return msg.send(" You must provide a prefix to remove!");

    const prefix = msg.author.settings.prefix;
    if (!prefix.includes(prefixInput))
      return msg.send(" That prefix is not in your list.");
